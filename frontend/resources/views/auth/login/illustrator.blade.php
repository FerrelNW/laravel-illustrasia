...
@section('body')
    <div
        ...
        <h2 class="text-sm text-slate-700 text-left px-4 mt-6">Don't have an account? <a href="{{ route('register.illustrator') }}"
                class="underline text-yellow-500">Sign up</a></h2>
        <form class="mx-auto px-4 w-full mt-3" method="POST" enctype="multipart/form-data"
            action="{{ route('login.illustrator') }}">
            @csrf
...
@section('script')
    <script>
        document.title = 'Login Illustrator'
    </script>
@endsection
